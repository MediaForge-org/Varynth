...
- `Varynth.Core.Simulation` — Wirtschaftssimulation, Bevölkerung, Produktion, Logistik, Forschung, Diplomatie/KI, Utilities, Disasters, Expeditionen; Engine-unabhängig, wo möglich reines C#. Fundament seit Phase 1C: `GameClock`/`GameTick` (deterministischer Tick-Zähler, keine Wall-Clock-/FPS-Kopplung), `SimulationScheduler`/`ISimulationSystem` (deterministische Systemreihenfolge), `SimulationContext`/`SimulationLevel`/`SimulationLevelMask` (ActiveNear/ActiveFar/Background), `PlayerId`/`ISimulationCommand` (Multiplayer-Future-Proofing, siehe unten). **Erweitert seit Phase 2E** um die tatsächlich engine-freie autoritative Simulation Boundary: `ISimulation`/`ManagedSimulation` (koordiniert die inhaltlich unveränderten `ArchipelagoPlacementState`/`RoadNetworkState`/`BuildingPlacementCommandHandler`/`RoadCommandHandler`, die von `Varynth.World` hierher verschoben wurden, da sie bis auf wenige mechanische Stellen — `Terrain`-Parameter, `RectInt`, `Mathf`/`Vector2` — bereits engine-frei waren), `ISimulationPlacementQueries`/`ISimulationRoadQueries` (schmale Presentation-Lesegrenze), `SimulationSnapshot`/`BuildingRenderSnapshot`/`RoadRenderSnapshot` (Double-Buffering, Tick vs. StateVersion getrennt), `SimulationCommandTicket`/`CommandEnvelope`/`BuildingCommandResult`/`RoadCommandResult`, `SimulationTickConfig`/`FixedTickAccumulator` (fester Simulationstick, spiralensicher geklemmt). Siehe `PHASE_2E_SIMULATION_BOUNDARY_FOUNDATION.md`. Referenziert weiterhin nur `Varynth.Core.Definitions`, `noEngineReferences: true`.
- `Varynth.World` — **implementiert seit Phase 2A** (Grid-/Terrain-Fundament: `WorldGrid`, `GridMeshBuilder`, `IWorldHeightSource`/`UnityTerrainHeightSource`, `IslandHeightmapGenerator`, `WorldPointer`); **erweitert seit Phase 2B** um Multi-Insel-Unterstützung (`CompositeWorldHeightSource`, `WorldPointer` über mehrere registrierte Collider) und eine kompakte Surface-Classification-Grundlage (`Varynth.World.Surface`: `SurfaceCellFlags`, `IslandSurfaceMap`, `SlopeEstimator`, `SurfaceMapGenerator`, `ResourceCandidateGenerator`, `VegetationCandidateGenerator`); **erweitert seit Phase 2C** um `Varynth.World.Placement` (Footprint-/Rotations-Math, `IslandOccupancyMap`, `PlacementValidator`, `ArchipelagoPlacementState` — command-agnostisches Welt-Bau-State, aus laufzeitfähigen `IslandSurfaceRuntimeData`-Assets aufgebaut, nicht aus Editor-only-Typen — plus `BuildingPlacementCommandHandler`, dem einzigen Ort mit einer neuen, eng begrenzten Referenz auf `Varynth.Core.Simulation`); **erweitert seit Phase 2D** um `BuildingRepeatPlanner` (deterministischer Drag/Repeat-Origin-Generator) sowie `Varynth.World.Roads` (`RoadGraph`/`RoadNetworkState` — command-agnostisches, zu `ArchipelagoPlacementState` referenzfrei gehaltenes Straßen-Welt-State; `RoadPlacementValidator`, `RoadRouter` (deterministisches Integer-A\*), `RoadMeshBuilder`, `BuildingRoadConnectionQuery`, `RoadCommandHandler`) — Cross-Validierung zwischen Gebäude- und Straßen-State läuft über die neutralen `IBuildingOccupancyQuery`/`IRoadOccupancyQuery`-Interfaces, nie über eine direkte Referenz der beiden State-Typen aufeinander. `RoadCommandHandler` nutzt dieselbe bereits bestehende `Varynth.Core.Simulation`-Referenz wie `BuildingPlacementCommandHandler`, keine neue Assembly-Kante nötig. Region-/Weltwechsel/Streaming-Koordination weiterhin offen für spätere Pakete. **Seit Phase 2E** verschoben nach `Varynth.Core.Simulation`: `ArchipelagoPlacementState`, `RoadNetworkState`, `BuildingPlacementCommandHandler`, `RoadCommandHandler` und der komplette engine-freie Placement-/Road-/Surface-Validierungscode (Namespaces unverändert `Varynth.World.Placement`/`Varynth.World.Roads`/`Varynth.World.Surface`, nur die Assembly-Zugehörigkeit hat sich geändert). `Varynth.World` behält `IslandSurfaceRuntimeData` (echtes `ScriptableObject`), `UnityTerrainHeightSource`, `RoadMeshBuilder`, `GridCellMeshBuilder`/`GridMeshBuilder` sowie den neuen `SimulationWorldBootstrap` (die einzige verbleibende Stelle, die zur Bootstrap-Zeit echtes `UnityEngine.Terrain` für autoritative Zwecke abtastet). `WorldGrid` liegt seit Phase 2E in `Varynth.Core.Common`. Referenziert `Varynth.Core.Definitions` (+ `Varynth.Core.Simulation`, jetzt für `ManagedSimulation`/`SimulationWorldBootstrap`-Aufruf statt nur für die beiden Command Handler).
- `Varynth.Presentation` — **implementiert seit Phase 2A** (Strategiekamera, Debug-Grid-Darstellung, Grid-Cell-Highlight, Welt-Interaktion/Input); **erweitert seit Phase 2C** um `Varynth.Presentation.Placement` (`PlacementController`, `PlacementGhostDisplay`, Prototype-Bauleiste) — erste echte Nutzung der Command-Boundary (`PlaceBuildingCommand`/`RemoveBuildingCommand`). Instancing/LOD/VFX weiterhin offen; **erweitert seit Phase 2D** um `Varynth.Presentation.Roads` (`RoadPlacementController`, `RoadPreviewDisplay`, `RoadNetworkDisplay` — genau ein zur Laufzeit geklonter, wiederverwendeter Mesh pro Insel, nie das gespeicherte Editor-Asset direkt mutiert) sowie `DragPreviewDisplay` (gepoolter Multi-Ghost, zwei Meshes statt einem GameObject pro geplantem Gebäude) und `ConstructionToolCoordinator`/`ConstructionToolCoordinatorHost` (zentrale Tool-Aktivierung/-Abbruch und Player-Placement-Grid-Sichtbarkeit; `PlacementController` und `RoadPlacementController` referenzieren sich dadurch nie direkt gegenseitig). **Erweitert seit Phase 2E** um `UnitySimulationDriver` (der einzige Unity-spezifische Baustein: übersetzt `Time.unscaledDeltaTime` über `FixedTickAccumulator` in `ISimulation.AdvanceTicks`-Aufrufe, besitzt die eine `ManagedSimulation`-Instanz). `PlacementController`/`RoadPlacementController` konstruieren `ArchipelagoPlacementState`/`RoadNetworkState` nicht mehr selbst, sondern finden den Driver wie `ConstructionToolCoordinatorHost` über `FindFirstObjectByType` in `Start()` und hängen ausschließlich an `ISimulationPlacementQueries`/`ISimulationRoadQueries`/`ISimulation` — nie an der konkreten `ManagedSimulation`-Klasse. Bestätigte Mutation läuft über `Submit(...)`; das Spawnen/Despawnen von Gebäude-GameObjects und der Road-Mesh-Rebuild laufen über einen Snapshot-Diff (`BuildingStateVersion`/`RoadStateVersion`-gated), nie mehr synchron direkt nach einem Command-Handler-Aufruf. Referenziert `Varynth.World`, `Varynth.Core.Definitions`, `Unity.InputSystem`, seit Phase 2C zusätzlich `Varynth.Core.Simulation` (Command-Konstruktion, seit Phase 2E auch `ISimulation`/`ManagedSimulation`); liest Simulationszustand weiterhin nur read-only.
...
**Seit Phase 2E real umgesetzt** (nur die technische Fixed-Tick-Grundlage, keine Balancing-Entscheidung): `ManagedSimulation` läuft in diskreten Ticks, angetrieben von `UnitySimulationDriver` über `FixedTickAccumulator`. Aktuelle Tickrate ist ein dokumentierter Prototypwert (`SimulationTickConfig.TicksPerSecond = 20.0`, `MaxCatchUpTicksPerFrame = 10`), zentral konfigurierbar, ausdrücklich nicht final — die obige Unterscheidung in Nah-/Fern-/Hintergrundsimulation ist damit architektonisch weiterhin vorbereitet (`SimulationLevel`/`SimulationLevelMask` existieren bereits seit Phase 1C), aber noch nicht mit unterschiedlichen Tickraten pro Ebene verdrahtet.
